...
const allQuestions = {
    // --- JUNGLE PROFESSOR QUESTIONS (3 questions per animal) ---
    ...
    // --- CITY TOUR QUESTIONS (3 questions per landmark) ---
    ...
    'c_rushmore2': { id: "c_rushmore2", question: "MOUNT RUSHMORE: Name one of the four US presidents carved into the mountain.", answer: "lincoln" }, // Others: Washington, Jefferson, Roosevelt
        ...
        if (data.method === "play") {
            ...
            if (positionData) {
                setTimeout(() => {
                    ...
                    const questionSet = questionIdSet.map(id => allQuestions[id]).filter(q => q); // .filter(q => q) removes undefined questions
